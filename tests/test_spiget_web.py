...
def test_get_resources():
    resources = get_resources(20)
    assert len(resources) == 20


def test_get_resource():
    commons = get_resource('15290')
...
def test_get_specific_version():
    commons = get_resource('15290', version='1.8.8-2')
    assert commons['version'] == '1.8.8-2'


def test_get_version_download_link():
    latest = get_resource_latest_version('15290')
    download_url = get_resource_download('15290')
    assert 'commons.15290' in download_url


def test_get_version_download_link_version_specific():
    download_url = get_resource_download('15290', version='1.8.8-1')
...
def test_get_resource_author():
    author = get_resource_author('15290')
...
def test_get_new_resources():
    new_resources = get_new_resources()
...
def test_get_category_specific_information():
    bukkit_category = get_categories('bukkit')
...
def test_get_category_resources():
    bukkit_category_resources = get_category_resources('bukkit', 5)
    assert len(bukkit_category_resources) == 5


def test_get_authors():
    authors = get_authors(10)
    assert len(authors) == 10


def test_get_author_details():
    mumbos_hut = get_author_details('MumbosHut')
...
def test_get_author_resources():
    mumbos_hut_resources = get_author_resources('MumbosHut')
...
def test_get_new_authors():
    new_authors = get_new_authors(10)
    assert len(new_authors) > 0


def test_resource_search():
    queried = search_resources('Commons')
